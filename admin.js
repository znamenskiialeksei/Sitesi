document.addEventListener('DOMContentLoaded', () => {
    let githubToken = null, currentConfig = null, selectedElementId = null;
    const DOM = {
        saveBtn: document.getElementById('save-btn'), canvas: document.getElementById('admin-canvas'),
    ...
    async function loadAdminPanel() {
        ...
        try {
            ...
            if (response.status === 404) {
                const liveUrl = `https://${window.location.hostname}${window.location.pathname.replace('admin.html', '')}config.json`;
                alert(`Критическая ошибка: Файл config.json не найден!\n\nПроверьте:\n${liveUrl}`);
                throw new Error("Файл config.json не найден (404)");
            }
            ...
            if (!error.message.includes("404")) alert(`Критическая ошибка загрузки: ${error.message}`);
        }
    }
    ...
    function renderCanvas() {
        ...
        DOM.canvas.style.backgroundColor = mainLayout.background?.type === 'color' ? mainLayout.background.value : '#ffffff';
        canvasMain.style.gap = mainLayout.columnGap || '20px';
    ...
    function renderGlobalSettingsPanel() {
        ...
        body.querySelector('input').addEventListener('input', e => { currentConfig.globalSettings.pageTitle = e.target.value; });
    }
    ...
    function updateElementFromInspector(event) {
        ...
        const value = input.type === 'checkbox' ? input.checked : input.value; // Handle checkboxes
        ...
        else if (input.dataset.styleKey) { if (!elementData.styles) elementData.styles = {}; elementData.styles[input.dataset.styleKey] = value; }
    ...
    function makeElementsResizable() {
        interact('.admin-element-wrapper').resizable({
            ...
            listeners: {
                move(event) { const target = event.target; target.style.width = `${event.rect.width}px`; target.style.height = `${event.rect.height}px`; },
                end(event) {
    ...
    function urlToEmbed(url) {
        ...
        if (instaMatch) return `https://www.instagram.com/reel/${instaMatch[1]}/embed/`;
        // TODO: Добавить конвертеры для других сервисов (YouTube, OneDrive и т.д.)
        return url;
    }
    ...
    function createElement(elementData) {
        ...
        switch (elementData.type) {
            case 'externalBlock': case 'videoBlock': case 'reels':
                ...
                // Задача 4 и 5: Используем конвертер и исправляем отображение
            ...
            case 'button': element = document.createElement('button'); element.textContent = elementData.content.text; element.style.pointerEvents = 'none'; element.title = "Эта кнопка станет кликабельной на публичной странице сайта."; break;
    ...
    function renderInspector(elementId) {
        ...
        inspectorBody.querySelector("#delete-element-btn").addEventListener("click", deleteSelectedElement);
        // Задача 8: Дополнительная логика для редактора выпадающего списка
        if (elementData.type === 'button') {
            const dropdownToggle = inspectorBody.querySelector('#dropdown-toggle');
            dropdownToggle?.addEventListener('change', () => {
                elementData.content.hasDropdown = dropdownToggle.checked;
                if(dropdownToggle.checked && !elementData.content.dropdownItems) elementData.content.dropdownItems = [];
                renderInspector(elementId);
            });
            inspectorBody.querySelector('#add-dropdown-item')?.addEventListener('click', () => {
                elementData.content.dropdownItems.push({label: 'Новый пункт', url: '#'});
                renderInspector(elementId);
            });
            inspectorBody.querySelectorAll('.delete-dropdown-item').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    elementData.content.dropdownItems.splice(e.target.dataset.index, 1);
                    renderInspector(elementId);
                });
            });
             inspectorBody.querySelectorAll('.dropdown-item-editor input').forEach(input => {
                input.addEventListener('input', (e) => {
                    const { index, key } = e.target.dataset;
                    elementData.content.dropdownItems[index][key] = e.target.value;
                });
            });
        }
    }
    ...
    function generateContentFields(element) {
        const urlNote = `<p style="font-size:12px;color:#777;">Для Instagram/YouTube используйте обычную ссылку. Для других сервисов (OneDrive) может понадобиться специальная 'embed' ссылка.</p>`;
        switch(element.type){
            case "photo": case "videoBlock": case "externalBlock":
                return `<div class="inspector-field"><label>URL</label><input type="text" data-content-key="url" value="${element.content.url||""}"></div>${urlNote}`;
            case "reels":
                return `<div class="inspector-field"><label>URL</label><input type="text" data-content-key="url" value="${element.content.url||""}"></div>${urlNote}
                        <div class="inspector-field"><label>Соотношение сторон</label><select data-style-key="aspectRatio"><option value="">Авто</option><option value="9/16" ${element.styles?.aspectRatio==='9/16'?"selected":""}>Вертикальное (9:16)</option><option value="1/1" ${element.styles?.aspectRatio==='1/1'?"selected":""}>Квадратное (1:1)</option><option value="16/9" ${element.styles?.aspectRatio==='16/9'?"selected":""}>Горизонтальное (16:9)</option></select></div>`;
            ...
            case "button":
                // Задача 8: Новый интерфейс для кнопки
                const dropdownEditor = element.content.hasDropdown
                    ? `<div class="inspector-group">
                         <h4>Пункты списка</h4>
                         <div id="dropdown-items-editor">
                           ${element.content.dropdownItems?.map((item, index) => `
                             <div class="dropdown-item-editor">
                               <input type="text" placeholder="Текст" data-index="${index}" data-key="label" value="${item.label}">
                               <input type="text" placeholder="URL" data-index="${index}" data-key="url" value="${item.url}">
                               <button class="delete-dropdown-item" data-index="${index}">❌</button>
                             </div>
                           `).join('') || ''}
                         </div>
                         <button id="add-dropdown-item" style="width:100%;margin-top:5px;">+ Добавить пункт</button>
                       </div>`
                    : `<div class="inspector-field"><label>Действие</label><select data-content-key="action"><option value="openLink" ${element.content.action==="openLink"?"selected":""}>Открыть ссылку</option><option value="openModal" ${element.content.action==="openModal"?"selected":""}>Модальное окно</option></select></div>
                       <div class="inspector-field"><label>URL</label><input type="text" data-content-key="url" value="${element.content.url||""}"></div>
                       <div class="inspector-field"><label>HTML модального окна</label><textarea data-content-key="modalContent">${element.content.modalContent||""}</textarea></div>`;

                return `<div class="inspector-field"><label>Текст кнопки</label><input type="text" data-content-key="text" value="${element.content.text||""}"></div>
                        <div class="inspector-field"><label><input type="checkbox" id="dropdown-toggle" data-content-key="hasDropdown" ${element.content.hasDropdown?"checked":""}> Выпадающий список</label></div>
                        ${dropdownEditor}`;
            default: return"<p>Нет настроек.</p>"
        }
    }
    ...
    function makePanelsInteractive(){interact(".floating-panel").draggable({allowFrom:".panel-header",inertia:true,modifiers:[interact.modifiers.restrictRect({restriction:"parent",endOnly:true})],listeners:{move(event){const target=event.target;const x=(parseFloat(target.getAttribute("data-x"))||0)+event.dx;const y=(parseFloat(target.getAttribute("data-y"))||0)+event.dy;target.style.transform=`translate(${x}px, ${y}px)`;target.setAttribute("data-x",x);target.setAttribute("data-y",y)}}});document.querySelectorAll(".panel-action").forEach(btn=>{btn.addEventListener("click",function(){const panel=this.closest(".floating-panel");const action=this.dataset.action;if(action==="close")panel.style.display="none"})})}
